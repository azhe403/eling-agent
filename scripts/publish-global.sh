...
echo "-- dotnet publish src/backend/Eling.Backend"
dotnet publish "$REPO_ROOT/src/backend/Eling.Backend" \
...
# Stop existing running processes if any (single binary now)
if [ "$IS_WINDOWS" = true ]; then
  taskkill //F //IM eling-backend.exe >/dev/null 2>&1 || true
...
cp -f "$OUT_DIR/eling-backend$EXE_EXT" "$BIN_DIR/"
chmod +x "$BIN_DIR/eling-backend$EXE_EXT" 2>/dev/null || true
# Keep old eling$EXE_EXT as symlink for backwards-compat
if [ "$IS_WINDOWS" != true ]; then
  ln -sf "$BIN_DIR/eling-backend$EXE_EXT" "$BIN_DIR/eling$EXE_EXT" 2>/dev/null || cp -f "$BIN_DIR/eling-backend$EXE_EXT" "$BIN_DIR/eling$EXE_EXT" 2>/dev/null || true
fi
...
ELING_BIN="$BIN_DIR/eling-backend$EXE_EXT"
...
echo "  health:           $HEALTH"
echo "  binary:            $BIN_DIR/eling-backend$EXE_EXT"
